...
router.get('/getUserById', checkToken, function (req, res, next) {
  const {_id} = req.query
  UserModel.findOne({ "_id": _id }, { lastname: 1, workcode: 1, hiredate: 1, position: 1, level: 1, gender: 1, _id: 1, hiredata: 1, depid: 1, actConfData: 1, costcenter: 1, houseAllance: 1, isCN: 1, isConf: 1, isTech: 1, mealAllance: 1, national: 1, remark: 1, salGroup: 1 }).then((data) => {
    if (!data) {
      res.json({
        code: '3001',
        msg: '数据为空'
...
router.put('/updateUserById', checkToken, function (req, res, next) {
  console.log('@@',req.body)
  let { _id,...updateData } = req.body;
  console.log('@',updateData)
  UserModel.updateOne({ "_id":  _id},{$set:updateData}).then((data) => {
    if (!data) {
      res.json({
        code: '3001',
        msg: '数据为空'
